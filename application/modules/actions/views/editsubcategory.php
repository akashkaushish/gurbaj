...
<form class="settings" onsubmit="return validateForm()" name="myForm" action="<?php  echo site_url('audio/admin/editsubcat/'.$id)?>" method="post" enctype="multipart/form-data">
		
		<ul>
			<li><input type="submit" name="submit" value="<?php  echo "Save"; ?>" class="input-submit" /></li>
			<li><a href="<?php  echo site_url('audio/admin/subcategorylist')?>" class="input-submit last"><?php  echo "Cancel"; ?></a></li>
		</ul>
		...
		<div id="one">
			 ...
			 <label for="parent_cat_id"><?php  echo "Parent Category*"; ?>: </label>
			...
			<?php if(!empty($parentcategory)){
			   foreach($parentcategory as $key=>$data){
				   ...
				   $selected = ($category['parent_cat_id'] ==$data['id'])?"selected='selected'":""
			   	?>
			...
			<?php $image = $category['category_image']; 
			   if($image !=""){ echo "
			   <label for='category_image'></label>
			   <img src ='".base_url()."/media/soundsubcategory/".$image."'>"; echo "<br>"; }
			?>
