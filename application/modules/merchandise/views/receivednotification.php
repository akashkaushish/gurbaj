    ...
    <div class="media-aset">
      ...
      <div class="col-2">
       <?php /*?> <?php if ( $media['media_type_id']==1){?>
        <a  style="text-align:center" href="<?php  echo site_url('member/mediadetail/'.$media['user_media_id'])?>" data-lity><img src="<?=base_url()?>media/media_type/photo/thumb/<?php echo  $media['media']; ?>" alt="Photo"></a>
        <?php }else if($media['media_type_id']==2){  ?>
        <a  style="text-align:center" href="<?php  echo site_url('member/mediadetail/'.$media['user_media_id'])?>" data-lity><img src="<?=base_url()?>application/views/<?php echo $this->system->theme_dir . $this->system->theme ?>/images/video-icon-list.png" alt="video"></a>
      ...
      <div class="col-4">
       <?php /*?> <?php if ( $_SESSION['is_coach']==1 && $_SESSION['is_paid']==1 ){?>
        ...
        <?php } ?><?php */?>
       <a href="<?php  echo site_url('member/mediadetail/'.$media['user_media_id'])?>" data-lity>View</a><a href="<?php  echo site_url('member/deleterecord_recevied/'.$media['user_media_id'])?>" onclick="return confirm('Are you sure you want to delete this file?');" class="<?php echo $media['user_media_id']?>">Delete</a> </div>
    </div>
    <?php endforeach; ?>
	<div class="pagging-new"><ul><?php  echo $pager?></ul></div>
	<?php }else{?>
	<div class="media-aset"><p>You haven't received any notification.</p></div>
