		...
		if ( isset($_SESSION['user_id']) &&  $_SESSION['user_id'] > 0)
		{
			...
			$this->template['totalwallet'] = $amount_in_wallet;
				...
				{	
					...
					{
						$this->session->set_flashdata('error', "You do not sufficient amount in your wallet.");
						...
						//sent email with approval code
						 if($transaction_request_id){
						        ...
						        $update_request = $this->member_model->update_transaction_request($transaction_request_id,$keydata);
								    ...
								    $this->session->set_flashdata('success', "You will receive a verification code via email and after verification your tranfer request will be approved.");
	...
	{	$this->template['selectpage'] = 4;
		...
		if ( isset($_SESSION['user_id']) &&  $_SESSION['user_id'] > 0)
		{
			...
			if($alreadyrequestsent[0]['num'] > 0 ){
			  $this->session->set_flashdata('error', "You already sent a withdraw request to admin, you have to wait untill admin will approve your previous request.");
			...
			{	
					...
					if($amount > $this->template['wallet_total'])
					{
						...
						if($request_id > 0)
						{
									...
									$message = 'You have initiated a withdrawrequest of amount $'.$amount.'. <br><br>The OTP for this withdrawrequest is <br><br>'.$key.' <br><br>To verify your withdrawrequest please click in the button below';
...
/*	function payout()
   ...
   function showlevel($start = 0, $limit = 20, $order = 'id')
	...
	{	$this->template['selectpage'] = 4;
		...
		if ( isset($_SESSION['user_id']) &&  $_SESSION['user_id'] > 0)
		{
			$this->template['upcomingpayout'] = $this->member_model->get_user_upcoming_payout($_SESSION['user_id']);	
