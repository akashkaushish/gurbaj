  ...
  function royaltypayment($start = 0, $limit = 20, $order = 'id') 
      {
		...
		function reports()
		{
			if ($this->input->post('month') && $this->input->post('year')){ 
				$this->template['month'] = $this->input->post('month'); 
				$this->template['year'] = $this->input->post('year'); 
			}else{ 
				$this->template['month'] = date('m'); 
				$this->template['year'] = date('Y'); 
			}
			$date_month = $this->template['year'].'-'.$this->template['month']; 
			$this->template['business'] = $this->member_model->get_total_business_by_month($date_month);
			$this->layout->load($this->template, 'reports');
			return;
		}
...
function inserttesting($user_id,$level,$plan_id,$planactivedate,$commissionamount,$by_user_id,$by_user_name){ 

    // $onemonth = date('Y-m-d', strtotime('+1 month', strtotime($planactivedate)));
	$last = date("Y-m-t", strtotime($planactivedate)); 

	for($j=1;$j<=4;$j++){
	
			$nextmonth = $this->endCycle($last, $j);
			echo $nextmonthend = date("Y-m-t", strtotime($nextmonth));echo "<br>";
		
		 // $lastdat= date("Y-m-t", strtotime($onemonth));
		//echo $lastdat; exit;	
		//echo $monthdatenext = date('Y-m-d', strtotime('+'.$j.' month', strtotime($lastdat)));echo "<br>";
		//echo $effectiveDate = date('Y-m-d', strtotime("+ ".$j." months", strtotime($lastdat)));echo "<br>";
		//echo $monthdate = date("Y-m-t", strtotime($monthdatenext)); echo "<br>";
...
$amount=1000;
 $level=array('1'=>2,'2'=>1,'3'=>1,'4'=>.5,'5'=>.5,'6'=>.25,'7'=>.25);
		...
		$user_id1 = $by_user['ref_id'];
			if($user_id1 > 0){
			 $commissionamount1=($amount/100)*$level[1];
			 
			// echo $user_id1.'/'.$plan_id.'/'.$planactivedate.'/'.$commissionamount1.'/'.$by_user_id.'/'.$by_user_name; exit;
			
				$levelone=$this->inserttesting($user_id1,1,$plan_id,$planactivedate,$commissionamount1,$by_user_id,$by_user_name);
			 }


}

function insertlevelcommission($user_id,$level,$plan_id,$planactivedate,$commissionamount,$by_user_id,$by_user_name){

   $last = date("Y-m-t", strtotime($planactivedate)); 
   
	for($j=1;$j<=4;$j++){
		$userdata1=$this->member_model->get_user_by_id1($user_id);
		$monthdate='';	
		
		//$lastdat= date("Y-m-t", strtotime($planactivedate));
		
		//echo $lastdat; exit;	
		//$monthdatenext = date('Y-m-d', strtotime('+'.$j.' month', strtotime($lastdat)));
		
		//$monthdate = date("Y-m-t", strtotime($monthdatenext)); 
		
		
		$monthdatenext = $this->endCycle($last, $j);
		$monthdate = date("Y-m-t", strtotime($monthdatenext));
		
		
		//enter data in  ci_point_user_transaction
		$userpoint_transaction = array('user_id' =>$user_id, 'commission_amount' =>$commissionamount,'commission_month' =>$monthdate,'commission_percentage'=>1,'is_paid' =>0,'comm_by_user_id' =>$by_user_id,'comm_by_username' =>$by_user_name,'comm_by_plan_id' =>$plan_id,'comm_by_level' =>$level,'date_created' =>@date("Y-m-d"));
		...
		if($insert_point_transaction)
		{
			...
			$this->member_model->update_user($userdata1[0]['id'], $updateuserdata);
		}
	}
  ...
  function mynew($by_user_id,$plan_id,$amount,$planactivedate){ 
		...
		$user_id1 = $by_user['ref_id'];
			if($user_id1 > 0){
			 $commissionamount1=($amount/100)*$level[1];
				...
				$levelone=$this->insertlevelcommission($user_id1,1,$plan_id,$planactivedate,$commissionamount1,$by_user_id,$by_user_name);
			 }
			...
			/*	$data1 = array(
				...
				if($user_id2 > 0){
				 ...
				 $commissionamount2=($amount/100)*$level[2];
					
					$level2=$this->insertlevelcommission($user_id2,2,$plan_id,$planactivedate,$commissionamount2,$by_user_id,$by_user_name);
					//Level 3
					$user_id3 = $this->member_model->get_user_referer_id($user_id2);
					if($user_id3 > 0){
					$commissionamount3=($amount/100)*$level[3];
						$data3 = array(
							'user_id' => $user_id3,
							'plan_id' => $plan_id,
							'amount' => $amount,
						...
						$level3=$this->insertlevelcommission($user_id3,3,$plan_id,$planactivedate,$commissionamount3,$by_user_id,$by_user_name);
						//Level 4
						$user_id4 = $this->member_model->get_user_referer_id($user_id3);
						if($user_id4 > 0){
						$commissionamount4=($amount/100)*$level[4];
							
						$levelone4=$this->insertlevelcommission($user_id4,4,$plan_id,$planactivedate,$commissionamount4,$by_user_id,$by_user_name);
							
							//Level 5
							$user_id5 = $this->member_model->get_user_referer_id($user_id4);
							if($user_id5 > 0){
							$commissionamount5=($amount/100)*$level[5];
							
								$levelone5=$this->insertlevelcommission($user_id5,5,$plan_id,$planactivedate,$commissionamount5,$by_user_id,$by_user_name);
								//Level 6
								$user_id6 = $this->member_model->get_user_referer_id($user_id5);
								if($user_id6 > 0){
								    $commissionamount6=($amount/100)*$level[6];
									
									$levelone6=$this->insertlevelcommission($user_id6,6,$plan_id,$planactivedate,$commissionamount6,$by_user_id,$by_user_name);
									
									//Level 7
									$user_id7 = $this->member_model->get_user_referer_id($user_id6);
									if($user_id7 > 0){
									   $commissionamount7=($amount/100)*$level[7];
										
										$levelone7=$this->insertlevelcommission($user_id7,7,$plan_id,$planactivedate,$commissionamount7,$by_user_id,$by_user_name);
  ...
  function testbounus(){
   ...
   $this->mynew('135','4','1000','2020-06-25');
   echo 'done'; exit;
  
  }
  
  function mynew_old($id,$planid,$amount,$planactivedate){
 ...
 function showbonus($start = 0, $limit = 20, $order = 'id') 
	...
	function unconfirmeduser($start = 0, $limit = 20, $order = 'id') 
      {
			...
			$this->template['members'] = $this->member_model->get_unconfirmed_user_payment($filter, array('start' => $start,'limit' => $limit, 'order_by' => $order));
			//echo "<pre>"; print_r($this->template['members']); exit;
			$this->load->library('pagination');
			$config['uri_segment'] = 4;
			$config['first_link'] ='First';
			$config['last_link'] = 'Last';
			$config['base_url'] = site_url('member/admin/unconfirmeduser');
			$config['total_rows'] = $this->member_model->get_unconfirmed_user_payment_count($filter);
			$config['per_page'] = $limit; 
			$this->pagination->initialize($config); 
			$this->template['pager'] = $this->pagination->create_links();
			$this->template['start'] = $start;	
			$this->layout->load($this->template, 'unconfirmeduser');
			return;
		}		
	...
	{	
		if($user_plan_id > 0)
		{
			...
			$user = $this->member_model->get_user_by_id($user_id); //echo "<pre>"; print_r($user); exit;
							...
							if($month_paid_status == 0)
							{
								...
								if($payout_month_detail && $payout_month_detail['percent'] > 0)
								{
									...
									if($pay_month ==1)
									{
										...
										$unpaid_days_remaining = $todal_days_in_month-$days;
										...
										// 26th month entry start here
										if($unpaid_days_remaining > 0)
										{
											$paid_amount_days = ($paid_amount/$todal_days_in_month)*$unpaid_days_remaining;

											$month26 = $this->endCycle($last, 26);
											$monthend26 = date("Y-m-t", strtotime($month26));
											$data = array(
												'user_id' => $user_id,
												'plan_id' => $plan_id,
												'plan_price' => $plan_price,
												...
												'percentage' => $payout_month_detail['percent'],
												'pay_date' => $monthend26,
												'days' => $unpaid_days_remaining,
												'created_date' => $today	
												);
											$upcoming_payout_id = $this->member_model->insert_upcoming_payout($data);

										}
										// 26th month entry end here
									}else{
			...
			{				
				...
				else
				{
                  /*if ($this->member_model->exists(array('email' => $this->input->post('email'), 'id !=' => $user_id)))
					...
					$data = array('email' => $this->input->post('email'),'fname' => $this->input->post('fname'),'lname' => $this->input->post('lname'),'phone' => $this->input->post('phone'));
   ...
   {	
		if ($userplanid > 0)
		{
			...
			{ 
				...
				//set parent bonus
				$parentbonus=($plandata['price']/100);
					...
					$user_plandata=$this->member_model->user_plan_data($userplanid,$data);
					 ...
					 if($userdata['level_bonus'] < 7 ){
					  ...
					  if($bonus <= 7 ){
					      ...
					      $data=array('user_plan'=> $plan_id, 'is_paid'=>1,'level_bonus'=>7);
					   }
				   ...
				   if(isset($userdata['ref_code_used']) && ($userdata['ref_code_used'] !='')){
					//get parent user data
					  ...
					  if($parentdata['level_bonus'] < 7){
					    /* $parentbonus=array('level_bonus'=>$parentdata['level_bonus']+1);*/
						 $newparentlevel=$parentdata['level_bonus']+$parentbonus;
						   if($newparentlevel <= 7){
						      $parentbonus=array('level_bonus'=>$newparentlevel);
						   }else{
						      $parentbonus=array('level_bonus'=>7);
						   }
					...
					$user_plandata=$this->member_model->user_plan_data($userplanid,$data);
					 ...
					 if($userdata['level_bonus'] < 7){
					   /* $data=array('user_plan'=> $plan_id, 'is_paid'=>1,'level_bonus'=>$bonus);*/
					     if($bonus <= 7 ){
					       $data1=array('user_plan'=> $plan_id, 'is_paid'=>1,'level_bonus'=>$bonus);
					     }else{
					       $data1=array('user_plan'=> $plan_id, 'is_paid'=>1,'level_bonus'=>7);
					     }
					   
					    $userdataupdate=$this->member_model->update_user($userdata['id'],$data1);
					   }
					//update parent user level
				   if(isset($userdata['ref_code_used']) && ($userdata['ref_code_used'] !='')){
					//get parent user data
 ...
 function veifyaccount($user_id=NULL)
   ...
   function addadminbrand()
	...
	function deletewithdraw($id)
	{
		...
		$this->session->set_flashdata("success", "Withdraw Request deleted");
		redirect('member/admin/withdrawrequests');
	}
  ...
  function deleteplan($id)
   {
			if ($id > 0)
             {
				$data=$this->member_model->get_userpayment_by_id($id); 		
				//delete from ci_user_business
				if(isset($data['user_id']) && ($data['user_id'])){
				   $userdata= $this->member_model->get_user_by_id($data['user_id']);	   
				   $this->db->delete('user_business', array('by_ID' => $userdata['username'],'is_confirmed'=>0,'plan_id'=>$data['plan_id']));				
				}
				//end
				
				//delete from the user plan
				$this->db->delete('user_plan_details', array('id' => $id));
				$this->session->set_flashdata("success", "Request deleted");
				redirect("member/admin/userpayment");
			}
			else
			{
				$this->session->set_flashdata("error", "Request does not exist.");
				redirect("member/admin/userpayment");
			}
      }		
