...
var ref_number = document.forms["myForm"]["ref_number"];
//var id_number = document.forms["myForm"]["id_number"];
    ...
    if (first_name.value == null || first_name.value == "") {
	 ...
	 if (passconf.value == null || passconf.value == "") {
    ...
    if (ref_number.value == null || ref_number.value == "") {
        alert("Referrer id must be filled out");
		ref_number.focus();
        return false;
    }
...
<form class="settings" onsubmit="return validateForm()" name="myForm" action="<?php  echo site_url('admin/member/create')?>" method="post" accept-charset="utf-8">
		...
		<div id="one">
			...
			<label for="reference_number"><?php  echo __("Referrer Id*", $module)?>:</label>
			<input type="text" name="ref_number" value="<?php  echo set_value('ref_number');?>" id="ref_number" class="input-text" /><br />
