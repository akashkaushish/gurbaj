...
class Page_Model extends CI_Model {
...

	{
		...
		$this->fields = array(
			...
			'pages' => array(
					...
					'lang'				=> "english",//$this->user->lang,

					'options'			=> '',

					'email'				=> (isset($this->user) && $this->user->email)?$this->user->email:"",
...

	{
		...
		$query = $this->db->get_where('tournament', array('id' => $id), 1, 0);
...

	{
		...
		$query = $this->db->get_where('media', array('media_id' => $id), 1, 0);
...

	{
		...
		$query = $this->db->get_where('projects', array('id' => $id), 1, 0);
	...
	*/
	...
	function get_project_by_projectcode($projectcode) {

	$query = $this->db->get_where('projects', array('project_code' => $projectcode), 1, 0);		
...

	{
		...
		$query = $this->db->get_where('pages', array('id' => $id), 1, 0);
		...
		function exists($fields)	{		$query = $this->db->get_where('projects', $fields, 1, 0);				if($query->num_rows() == 1)			return TRUE;		else			return FALSE;	}
